...
import cn.bizzan.R;
import cn.bizzan.adapter.CurrentEntrustAdapter;
...
import cn.bizzan.ui.credit.CreditActivity;
import cn.bizzan.ui.dialog.EntrustConstractDialogFragment;
import cn.bizzan.ui.entrust.TrustListConstractActivity;
import cn.bizzan.ui.home.presenter.SixImpl;
...
import cn.bizzan.widget.SixPopEntrustView;
import cn.bizzan.widget.SixPopModelView;
import cn.bizzan.widget.SixPopMultiplesView;
import cn.bizzan.widget.SixPopMultiplesView2;
    ...
    private int avaOpenSell() {
        try {
            ...
            if (data.getUsdtPattern() == "FIXED") { // 逐仓，直接usdt余额即可
                return (int) (data.getUsdtSellLeverage() * data.getUsdtBalance() / sixinfo.getShareNumber());
            } else {
                // 计算可用
                double buyPL = 0;
                if (data.getUsdtBuyPrice() > 0) {
                    buyPL = ((currency.getClose() / data.getUsdtBuyPrice() - 1) * (data.getUsdtBuyPosition() + data.getUsdtFrozenBuyPosition()) * data.getUsdtShareNumber());
                }
                double sellPL = 0;
                if (data.getUsdtSellPrice() > 0) {
                    sellPL = ((1 - currency.getClose() / data.getUsdtSellPrice()) * (data.getUsdtSellPosition() + data.getUsdtFrozenSellPosition()) * data.getUsdtShareNumber());
                }
                double lossandp = buyPL + sellPL + data.getUsdtBuyPrincipalAmount() + data.getUsdtSellPrincipalAmount();
                if (lossandp >= 0) {
                    return (int) (data.getUsdtSellLeverage() * data.getUsdtBalance() / sixinfo.getShareNumber());
                } else {
                    return (int) (data.getUsdtSellLeverage() * (data.getUsdtBalance() - lossandp) / sixinfo.getShareNumber());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private int avaOpenBuy() {
        try {
            ...
            if (data.getUsdtPattern().equals("FIXED")) { // 逐仓，直接usdt余额即可
                return (int) (data.getUsdtBuyLeverage() * data.getUsdtBalance() / sixinfo.getShareNumber());
            } else {
                // 计算可用
                double buyPL = 0;
                if (data.getUsdtBuyPrice() > 0) {
                ...
                if (lossandp >= 0) {
                    ...
                    return (int) (data.getUsdtBuyLeverage() * (data.getUsdtBalance() - lossandp) / sixinfo.getShareNumber());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        switch (view.getId()) {
            ...
            case R.id.ll_all_model:
                ...
                if (targetPattern.equals("1")) {
                    mPresenter.getSwitchPattern(sixinfo.getId() + "", "0",SharedPreferenceInstance.getInstance().getTOKEN());
                }
            ...
            case R.id.ll_chase_model:
                ...
                if (targetPattern.equals("0")) {
                    mPresenter.getSwitchPattern(sixinfo.getId() + "", "1",SharedPreferenceInstance.getInstance().getTOKEN());
                }
                sixpopmodelView.dismiss();
                break;
            ...
            case R.id.bu_true:
                ...
                if (mtabtype == 0) {
                    if (check == 0) {
                        mPresenter.getModifyLeverage(sixinfo.getId() + "", sixPopMultiplesView.getprogress(), "0",SharedPreferenceInstance.getInstance().getTOKEN());
                    } else if (check == 1) {
                        mPresenter.getModifyLeverage(sixinfo.getId() + "", sixPopMultiplesView.getprogress(), "1",SharedPreferenceInstance.getInstance().getTOKEN());
                    }
                } else if (mtabtype == 1) {
                    if (check == 0) {
                        mPresenter.getModifyLeverage(sixinfo.getId() + "", sixPopMultiplesView.getprogress(), "0",SharedPreferenceInstance.getInstance().getTOKEN());
            ...
            case R.id.bu_true2:
                ...
                if (mtabtype == 0) {
                    if (check == 0) {
                        mPresenter.getModifyLeverage(sixinfo.getId() + "", sixPopMultiplesView2.getnumber(), "0",SharedPreferenceInstance.getInstance().getTOKEN());
                    } else if (check == 1) {
                        mPresenter.getModifyLeverage(sixinfo.getId() + "", sixPopMultiplesView2.getnumber(), "1",SharedPreferenceInstance.getInstance().getTOKEN());
                    }
                } else if (mtabtype == 1) {
                    if (check == 0) {
                        mPresenter.getModifyLeverage(sixinfo.getId() + "", sixPopMultiplesView2.getnumber(), "0",SharedPreferenceInstance.getInstance().getTOKEN());
        ...
        switch (type) {
            case "LIMIT_PRICE":
                ...
                if (amout.contains("%")) {
                    ...
                    int zhang = 0;
                    String one = Zhang.getText().toString();
                    String two = Zhang2.getText().toString();
                    if (mtabtype == 0) {
                        if (check == 0) {
                            zhang =Integer.parseInt(one.substring(0,one.length() -1).trim());
                        } else {
                            zhang = Integer.parseInt(two.substring(0,two.length() - 1).trim());
                        }
                    } else {
                        if (check == 0) {
                            zhang = Integer.parseInt(two.substring(0,two.length() - 1).trim());
                    ...
                    double su = (Double.parseDouble(substring) / 100) * zhang;
                    if (su < 1) {
                        amout = 1 + "";
                    } else {
                        amout = (int) su + "";
