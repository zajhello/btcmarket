...
import cn.bizzan.serivce.MyTextService_contract;
...
import cn.bizzan.ui.option.OptionFragment;
import cn.bizzan.ui.option.SevenFragment;
import cn.bizzan.ui.option.presenter.SevenPresenter;
...
//    private LabFragment labFragment;
    ...
    private void tcpNotify() {
        ...
        //有新数据刷新下涨跌幅榜
        oneFragment.dataLoadedall(currenciesall);
...
//            presenter.find(getToken());
    ...
    public void allCurrencySuccess(Object obj) {
        try {
            ...
            if (oneFragment != null) oneFragment.dataLoaded(currency1);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @Override
    public void allCurrencySuccessall(Object obj) {

        try {
            List<Currency> objs = gson.fromJson((String) obj, new TypeToken<List<Currency>>() {
            }.getType());
            this.currenciesall.clear();
            this.currenciesall.addAll(objs);
            if (oneFragment != null) oneFragment.dataLoadedall(objs);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void setData() {
        oneFragment.dataLoaded(currenciesTwo);
        oneFragment.dataLoadedall(currenciesall);
