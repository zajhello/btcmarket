...
//            country = WonderfulToastUtils.getString(getActivity(), R.string.china);
            WonderfulToastUtils.showToast(R.string.select_region);
            return;
        } else {
...
//        presenter.signUpByPhone(phone, username, password, country, code,tuijianma,challenge,validate,seccode);
    ...
    private void getCode() {
        ...
        //应该是+86按照中国手机号验证码，其他地区只要不为空且是纯数字即可
        if (tvAreaCode.getText().toString().equals("+86")) {
            if (WonderfulStringUtils.isEmpty(phone) || phone.length() < 11) {
                WonderfulToastUtils.showToast(R.string.phone_not_correct);
                return;
            }
        } else {
            if (WonderfulStringUtils.isEmpty(phone)) {
                WonderfulToastUtils.showToast(R.string.phone_not_correct);
                return;
            }
        }
