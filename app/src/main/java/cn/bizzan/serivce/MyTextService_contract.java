...
//private static final String ip = "39.104.106.202";//行情 真实
    ...
    public void onGetMessage(SocketMessage message) {
        if (message.getCode() == 1) { // 合约方面的推送
            if (message.getCmd() == ISocket.CMD.SUBSCRIBE_EXCHANGE_TRADE_CONTRACT){
            ...
            synchronized (lock) {
                if (socket == null || socket.isConnected()) {
                    try {
                        ...
                        dos = new DataOutputStream(socket.getOutputStream());
                        isOpen = true;
                        WonderfulLogUtils.logi(TAG, "连接提示：2 连接成功2");
...
//                    System.out.println("111111:" + "111");
                    EventBus.getDefault().post(new SocketMessage(1, ISocket.CMD.CONTRACT_HEART_BEAT, null));
                    heart = 0;
                } else {
//                    System.out.println("111111:" + "222");
                    if (socket != null) try {
                        dis.close();
                        dos.close();
                        socket.close();
    ...
    private void startTCP() {
        ...
        if (message != null) {
//            System.out.println("111111:"+message.getCmd());
//            System.out.println("111111:"+message.getCode());
            EventBus.getDefault().post(message); // 开始订阅
        }
    }
}
