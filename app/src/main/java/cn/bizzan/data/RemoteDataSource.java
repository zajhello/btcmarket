...
import cn.bizzan.ui.mychart.KHuoBiLineBean;
...
public class RemoteDataSource implements DataSource {
    ...
    @Override
    public void KData(final String symbol, final Long from, final Long to, final String resolution, final DataCallback dataCallback) {
        WonderfulLogUtils.loge("INFO", from + "   " + to);
        //判断是走自己还是火币 1：走自己  ， 其他：火币
...
//                LogUtils.e("miao历史",response.toString());
    ...
    public void KHuoBi_DataGet(String symbol, Long from, Long to, String resolution, final DataCallback dataCallback) {
        ...
        WonderfulOkhttpUtils.get().url(UrlFactory.getHuoKlineData())
            ...
            @Override
            public void onResponse(String response) {
                WonderfulLogUtils.logi("历史K线数据回执：", "历史K线数据回执：" + response);
//                LogUtils.e("miao历史",response.toString());
                try {
                    ...
                    for (int i = kHuoBiLineBean.getData().size() - 1; i >= 0; i--) {
                        ...
                        jsonArray.put(datum.getClose());
                        jsonArray.put(datum.getVol());
...
//                LogUtils.e("miao历史",response.toString());
    ...
    public void allHomeCurrency(final DataCallback dataCallback) {
        WonderfulOkhttpUtils.post().url(UrlFactory.getAllCurrencys()).build().execute(new StringCallback() {
            ...
            public void onError(Request request, Exception e) {
                super.onError(request, e);
                WonderfulLogUtils.logi("获取币种出错", "获取币种出错：" + e.getMessage());
                dataCallback.onDataNotAvailable(OKHTTP_ERROR, null);
            }
...
//                Log.d("jiejie","home " + response);
                WonderfulLogUtils.logi("获取币种回执：", "获取币种回执：" + response.toString());
     ...
     */
    @Override
    public void allHomeCurrencyall(final DataCallback dataCallback) {
        WonderfulOkhttpUtils.post().url(UrlFactory.getAllCurrencysall()).build().execute(new StringCallback() {
            @Override
            public void onError(Request request, Exception e) {
                super.onError(request, e);
        ...
        WonderfulOkhttpUtils.post().url(UrlFactory.getTransUrl())
            ...
            public void onResponse(String response) {
                ...
                try {
                    JSONObject object = new JSONObject(response);
                    if (object.optInt("code") == 0) {
                        dataCallback.onDataLoaded("划转成功");
                    } else {
                        dataCallback.onDataNotAvailable(object.getInt("code"), object.optString("message"));
                    }
                } catch (JSONException e) {
